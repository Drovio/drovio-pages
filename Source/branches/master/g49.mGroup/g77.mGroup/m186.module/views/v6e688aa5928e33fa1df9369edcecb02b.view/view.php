...
$innerModules['designer'] = 365;
$innerModules['projectIconEditor'] = 381;
...
if (empty($projectName) && !empty($projectInfo['name']))
{
	...
	$url = url::resolve("developers", "/dashboard/".$projectName."/".$selectedTab);
...
// Build module page
$page->build($projectTitle, "projectHomePage", TRUE, TRUE);
...
if ($projectAdmin)
{
	...
	// Add action for project icon
	$projectImage = HTML::select(".projectHome .projectImage")->item(0);
	HTML::addClass($projectImage, "editable");
	$attr = array();
	$attr['id'] = $projectID;
	$actionFactory->setModuleAction($projectImage, $innerModules['projectIconEditor'], "", "", $attr);
}
...
foreach ($sections as $section)
{
	...
	if (empty($projectName))
	{
		...
		$url = url::resolve("developers", "/dashboard/".$projectName."/".$section."/");
