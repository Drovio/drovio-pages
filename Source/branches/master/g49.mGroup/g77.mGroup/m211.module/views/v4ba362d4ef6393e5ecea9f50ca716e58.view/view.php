...
$keys = apiKey::getProjectAccountKeys($projectID, $memberID);
	...
	else
	{
		...
		foreach ($_POST['tg'] as $typeID => $nothing)
		{
			...
			if (!isset($roles[$typeID]))
				apiKey::create($typeID, $memberID, $teamID = NULL, $projectID);
		}
	}
...
foreach ($keyTypes as $typeInfo)
{
	...
	if (!preg_match('/PROJECT_.*/', $typeInfo['user_group_name']))
	    continue;
