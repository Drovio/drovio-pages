...
if (engine::isPost())
{
	...
	// Set manifest private
	$private = (isset($_POST['private']) ? TRUE : FALSE);
	$mfManager->setPrivate($mfID, $private);
	
	// Set plans
	$premium = (isset($_POST['premium']) ? TRUE : FALSE);
	$enterprise = (isset($_POST['enterprise']) ? TRUE : FALSE);
	$mfManager->setPlans($mfID, $premium, $enterprise);
...
if (!empty($mfInfo['info']['title']))
{
	$hTitle = DOM::create("h2", $mfInfo['info']['title'], "", "hd title");
	DOM::append($infoContainer, $hTitle);
}
if (!empty($mfInfo['info']['description']))
{
	$hDesc = DOM::create("h4", $mfInfo['info']['description'], "", "hd desc");
...
// Project id
$input = $form->getInput($type = "hidden", $name = "id", $value = coreProject::PROJECT_ID, $class = "", $autofocus = "", $required = TRUE);
$form->append($input);
...
// Manifest private status
$title = moduleLiteral::get($moduleID, "lbl_mfPrivate");
$input = $form->getInput($type = "checkbox", $name = "private", $value = ($mfInfo['info']['private'] == 1), $class = "", $autofocus = "", $required = FALSE);
$form->insertRow($title, $input, $required = FALSE, $notes = "");

// Manifest private status
$title = moduleLiteral::get($moduleID, "lbl_mfPremium");
$input = $form->getInput($type = "checkbox", $name = "premium", $value = ($mfInfo['info']['premium'] == 1), $class = "", $autofocus = "", $required = FALSE);
$form->insertRow($title, $input, $required = FALSE, $notes = "");

// Manifest private status
$title = moduleLiteral::get($moduleID, "lbl_mfEnterprise");
$input = $form->getInput($type = "checkbox", $name = "enterprise", $value = ($mfInfo['info']['enterprise'] == 1), $class = "", $autofocus = "", $required = FALSE);
$form->insertRow($title, $input, $required = FALSE, $notes = "");
