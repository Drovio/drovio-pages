...
{	
	...
	$themeCSS = new templateThemeCSS($templateID, $_POST['tname']);
	$status = $themeCSS->create($_POST['css_name']);
	...
	if (!$status)
	{
		$err_header = literal::dictionary("name");
		$err = $errFormNtf->addHeader($err_header);
		$errFormNtf->addDescription($err, DOM::create("span", "Error addinc theme css..."));
		return $errFormNtf->getReport();
	}
