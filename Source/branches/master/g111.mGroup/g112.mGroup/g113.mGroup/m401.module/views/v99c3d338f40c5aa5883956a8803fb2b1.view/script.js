...
JSPreloader = {
	...
	loadStyle : function(styleName) {
		...
		// Create style url
		var styleSource = "https://api.drov.io/" + (this.dev == 1 ? "dev/" : "") + "apps/css/" + this.app_id + "/" + this.akey + "/" + styleName + "?origin=" + this.origin;
		
		// Load css style
		return jq("<link rel='stylesheet' href='"+styleSource+"'>").appendTo(jq("head"));
	},
	loadView : function(viewName, method, requestData, sender, successCallback, errorCallback, extraOptions) {
		// Create script url
		var viewLoaderUrl = "https://api.drov.io/" + (this.dev == 1 ? "dev/" : "") + "apps/" + this.app_id + "/" + this.akey + "/" + viewName;
	...
	request : function(ajaxUrl, method, requestData, sender, successCallback, ajaxOptions) {
		...
		// Reset origin (in case it's changed)
		this.origin = window.location.origin;
		
		// Add origin to request data
		if (requestData == null || typeof requestData == "undefined" || typeof requestData == "string")
			requestData += "&origin=" + this.origin;
		else
			requestData.append("origin", this.origin);
...
// Check if jQuery is loaded and load
if (!window.jQuery) {
	getScript("//cdn.drov.io/js/jquery/jquery-1.7.2.min.js", function() {
