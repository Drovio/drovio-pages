...
{	
	...
	// Create theme
	$theme = new wsTemplateTheme($websiteID, $_POST['tname']);
	...
	{
		$themeJS = new wsTemplateThemeJS($websiteID, $_POST['tname'], $themeName);
		$themeJS->create($_POST['js_name']);
	}
	
	if (!empty($_POST['css_name']))
	{
		$themeCSS = new wsTemplateThemeCSS($websiteID, $_POST['tname'], $themeName);
...
$templateResource = array();
foreach ($templates as $templateName)
	$templateResource[$templateName] = $templateName;
ksort($templateResource);

$title = moduleLiteral::get($moduleID, "lbl_templateName");
$label = $form->getLabel($title);
$input = $form->getResourceSelect($name = "tname", $multiple = FALSE, $class = "", $templateResource, $selectedValue = "");
