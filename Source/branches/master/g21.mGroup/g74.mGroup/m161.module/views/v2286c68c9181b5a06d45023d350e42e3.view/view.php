...
if (engine::isPost())
{
	...
	$valid = account::getInstance()->authenticate(account::getInstance()->getUsername(), $_POST['currentPassword']);
	if ($empty || !$valid)
	{
		$has_error = TRUE;
	...
	$status = account::getInstance()->updatePassword($_POST['currentPassword'], $_POST['newPassword']);
	...
	$attr = array();
	$attr['member_title'] = account::getInstance()->getAccountTitle();
	$subject = "Drovio password changed";
	$personMail = person::getInstance()->getMail();
	mMail::send("/mail/notifications/password_changed.html", $subject, $personMail, $attr);
