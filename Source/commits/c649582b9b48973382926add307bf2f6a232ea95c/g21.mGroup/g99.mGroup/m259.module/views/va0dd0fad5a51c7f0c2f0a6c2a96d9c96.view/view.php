...
{
	// Get navigation item
	$navItem = HTML::select(".myRelations .navItem.".$item)->item(0);
	$page->setStaticNav($navItem, $item, "relationsContainer", "relGroup", "rNavGroup", $display = "none");
	
	// Get target
	$target = HTML::select(".myRelations #".$item)->item(0);
