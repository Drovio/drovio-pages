...
$navItems = array();
$navItems["manifest"] = "applicationManifests";
$navItems["keys"] = "developerKeys";
foreach ($navItems as $item => $itemData)
{
	...
	// Get navitem
	$navItem = HTML::select(".appSecuritySettings .navBar .navTitle.".$item)->item(0);
	
	// Check if it is for preload
	$preload = HTML::hasClass($navItem, "selected");
	
	// Static navigation
	$page->setStaticNav($navItem, $ref, "settingsContainer", $targetgroup, "stNav", $display = "none");
