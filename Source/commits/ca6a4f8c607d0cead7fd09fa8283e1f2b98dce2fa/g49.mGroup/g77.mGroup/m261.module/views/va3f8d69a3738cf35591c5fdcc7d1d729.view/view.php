...
if (engine::isPost())
{
	...
	// Websites only from website publisher
	...
	if ($projectInfo['projectType'] == 5)
	{
		...
		// Add error content
		$errorContent = moduleLiteral::get($moduleID, "website_error_header");
		$pageContent->append($errorContent);
...
// Build Form
$form = new simpleForm();
$form->build("", FALSE)->engageModule($moduleID, "");
...
$currentVersion = (empty($currentVersion) ? "-" : $currentVersion);
...
$input = $form->getTextarea($name = "changelog", $value = "", $class = "", $autofocus = FALSE, $required = TRUE);
$inputRow = $form->buildRow($title, $input, $required = TRUE, $notes);
DOM::append($mainForm, $inputRow);
...
$title = moduleLiteral::get($moduleID, "lbl_publishProject");
$btnPublish = $form->getSubmitButton($title, $id = "btn_publish");
$form->appendControl($btnPublish);
...
$frame->build($title, "projectPublisherFrame", $background = TRUE);
