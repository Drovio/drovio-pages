...
// Resources switch
$switchRow = HTML::select(".testerPanel .switchRow.mrsrc")->item(0);
$switch = new switchButton("mrsrcTestSwitch");
$attr = array();
$attr['id'] = modulesProject::PROJECT_ID;
$switchObject = $switch->build("", mrsrcTester::status())->engageModule($moduleID, "mrsrcTesting", $attr)->get();
...
// Get developer and tester workspace modules
$devModules = developer::getWorkspaceModules();
$testerModules = tester::getWorkspaceModules();
...
// Logger log
$pagesLoggerContainer = HTML::select(".pagesLoggerContainer")->item(0);
if (!logger::status())
	HTML::addClass($pagesLoggerContainer, "noDisplay");

$logger = new logViewer();
$loggerViewer = $logger->build()->get();
DOM::append($pagesLoggerContainer, $loggerViewer);
