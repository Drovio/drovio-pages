...
// Load footer menu
$footerContainer = HTML::select(".sdkManualContainer")->item(0);
...
// Set top navigation
$navItem = HTML::select(".sdkManualContainer .objectExplorerContainer .topNav .hdNav")->item(0);
$page->setStaticNav($navItem, "coreSDK", $targetcontainer, $targetgroup, $navgroup, $display = "none");

$navItem = HTML::select(".sdkManualContainer .objectExplorerContainer .topNav .hdNav")->item(1);
$page->setStaticNav($navItem, "webSDK", $targetcontainer, $targetgroup, $navgroup, $display = "none");
...
$searchInput = HTML::select(".navigationContainer .searchInput")->item(0);
...
$packageExplorer = HTML::select(".sdkManualContainer #coreSDK")->item(0);
...
foreach ($libraries as $library)
{
	...
	buildPackageTree($navTree, $library, $sourceMap, $openPackages, $mfLibraries);
}
...
{
	// Load manual page
	$manualHolder = HTML::select(".sdkManualPage .manualContainer")->item(0);
...
function buildPackageTree($navTree, $libName, $sourceMap, $openPackages, $mfLibraries)
{
	$packages = $sourceMap->getPackageList($libName);
	foreach ($packages as $packageName => $value)
	{
		// Filter packages
		if (team::getTeamID() != 6 && !in_array($packageName, $openPackages[$libName]) && !isset($mfLibraries[$libName][$packageName]))
			continue;
...
function buildObjTree($navTree, $libName, $packageName, $parentNs, $sourceMap)
{
	...
	foreach ($objs as $key => $value)
	{
		...
		$GLOBALS["actFactory"]->setModuleAction($item, $GLOBALS["moduleID"], "manualViewer", ".manualContainer", $attr);
