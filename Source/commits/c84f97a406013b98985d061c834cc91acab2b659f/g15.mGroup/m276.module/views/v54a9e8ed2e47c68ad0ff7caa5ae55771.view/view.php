...
while ($folder = $dbc->fetch($folders))
{
	...
	$folderItem = DOM::create("div", "", "", "pgFolder");
	$folderIcon = DOM::create("span", "", "", "contentIcon fldIco");
	DOM::append($folderItem, $folderIcon);
	
	$description = ($folder['is_root'] ? $folder['domain'] : $folder['name']);
	$folderName = DOM::create("span", $description);
	DOM::append($folderItem, $folderName);
	
	// Check Parent
	$treeItem = $navTree->insertExpandableTreeItem("fld.".$folder['id'], $folderItem, "fld.".$folder['parent_id']);
...
foreach ($pages as $pageRow)
{
	// Create tree Item
	// Create group item container
	$item = DOM::create("div", "", "", "pgPage");
	$folderIcon = DOM::create("span", "", "", "contentIcon flIco");
	DOM::append($item, $folderIcon);
	
	$pageName = DOM::create("span", $pageRow["file"]);
	DOM::append($item, $pageName);
	...
	$navTree->assignSortValue($treeItem, $pageRow["file"]);
}
