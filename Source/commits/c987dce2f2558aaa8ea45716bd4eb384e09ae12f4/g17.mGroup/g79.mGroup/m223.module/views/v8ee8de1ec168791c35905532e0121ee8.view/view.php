...
{	
	...
	if ($status)
	{
		$success = $wsServer->updateInfo($_POST['description'], $_POST['type'], $_POST['connection'], $_POST['httpdocs'], $_POST['siteUrl']);
		if ($success)
		{
...
$input = $sForm->getInput($type = "text", $name = "siteUrl", $serverInfo['siteUrl'], $class = "", $autofocus = FALSE);
...
$input = $sForm->getInput($type = "text", $name = "password", $serverInfo['password'], $class = "", $autofocus = FALSE);
$row = $sForm->buildRow($title, $input, $required = FALSE, $notes = "");
DOM::append($group, $row);
