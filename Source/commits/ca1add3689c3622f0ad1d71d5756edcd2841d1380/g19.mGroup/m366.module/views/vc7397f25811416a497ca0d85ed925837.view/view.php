...
if (engine::isPost())
{
	...
	// Check team name
	$teamUName = str_replace(" ", "_", $_POST['uname']);
	if (empty($teamUName))
	{
		$has_error = TRUE;
	...
	if (empty($_POST['name']))
	{
		...
		// Header
		$err_header = moduleLiteral::get($moduleID, "lbl_teamFullName");
		$err = $errFormNtf->addHeader($err_header);
		$errFormNtf->addDescription($err, $errFormNtf->getErrorMessage("err.required"));
	}
...
$frame->build($title, "", $background = TRUE)->engageModule($moduleID, "createTeam");
$form = $frame->getFormFactory();

$title = moduleLiteral::get($moduleID, "lbl_teamName");
$notes = moduleLiteral::get($moduleID, "lbl_teamName_notes");
$input = $form->getInput($type = "text", $name = "uname", $value = "", $class = "", $autofocus = TRUE, $required = TRUE);
$fRow = $form->buildRow($title, $input, $required = TRUE, $notes);
$frame->append($fRow);

$title = moduleLiteral::get($moduleID, "lbl_teamFullName");
