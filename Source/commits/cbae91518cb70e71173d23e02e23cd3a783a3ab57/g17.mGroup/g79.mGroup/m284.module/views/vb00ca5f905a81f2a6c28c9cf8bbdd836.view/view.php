...
if (engine::isPost())
{
	...
	logger::log("Connecting to server : ".($status ? "SUCCESS" : "FAIL"), logger::DEBUG);
	
	// Navigate to httpdocs folder
	$status = $ftp->changeDir($serverInfo['httpdocs'], $createDir = TRUE);
	logger::log("Change to httpdocs dir : ".($status ? "SUCCESS" : "FAIL"), logger::DEBUG);
	...
	logger::log("Put prepare file : ".($status ? "SUCCESS" : "FAIL"), logger::DEBUG);
	...
	logger::log("Initiate setup process : ".(is_null($response) ? "FAIL: ".$error : "SUCCESS: ".$response), logger::DEBUG);
	...
	// Upload update file
	$status = $ftp->put($coreDir."/update.php", ".wec/scripts/update.php", '');
