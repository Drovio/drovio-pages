...
// Inner Module Codes
$innerModules = array();
$innerModules['accountProfile'] = 375;
$innerModules['teamProfile'] = 376;
...
$accountInfo = array();
if (!empty($profileID))
	$accountInfo = IDAccount::info($profileID);
else
	$accountInfo = IDAccount::getAccountByUsername($profileName, $includeEmail = FALSE);
...
$q = $page->getQuery("search_team");
$result = $dbc->execute($q, $attr);
if ($dbc->get_num_rows($result))
{
	// Get module container
	$accountInfo = $dbc->fetch($result);
	$mContainer = $page->getModuleContainer($innerModules['teamProfile'], $viewName = "", $attr, $startup = TRUE, $containerID = "companyProfilePageContainer", $loading = TRUE, $preload = TRUE);
	$page->append($mContainer);
