...
if (engine::isPost())
{
	...
	{
		// Delete account
		$result = managedAccount::remove($accountID);
	...
	if (!empty($_POST['password']))
	{
		...
		managedAccount::updatePassword($accountID, $_POST['password'], $_POST['admin_password']);
	}
	
	// Update account info
	$result = managedAccount::updateInfo($accountID, $_POST['title'], $_POST['description'], $_POST['username'], $_POST['locked'] == "on");
	if (!$result)
		return $errFormNtf->getReport();
...
// Get account info
$accountInfo = managedAccount::info($accountID);
