...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	$result = $mQuery->update($_POST['title'], $_POST['sqlQuery'], $_POST['description'], $_POST['attributes']);
...
$queryFormElement = $queryForm->build($moduleID, "moduleQueryEditor", $controls = FALSE)->get();

// Hidden Values
$input = $queryForm->getInput($type = "hidden", $name = "qid", $qID, $class = "", $autofocus = FALSE);
$queryForm->append($input);

$input = $queryForm->getInput($type = "hidden", $name = "mid", $mID, $class = "", $autofocus = FALSE);
$queryForm->append($input);
...
$input = $queryForm->getInput($type = "text", $name = "title", $mQ->getTitle(), $class = "", $autofocus = TRUE);
...
$input = $queryForm->getTextarea($name = "description", $mQ->getDescription(), $class = "");
