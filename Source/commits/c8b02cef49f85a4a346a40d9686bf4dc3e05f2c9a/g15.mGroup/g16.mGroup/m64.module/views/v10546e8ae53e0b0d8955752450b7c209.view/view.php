...
$pageContent->build("", "moduleExplorer");

$refreshToolbar = DOM::create("div", "", "", "refreshToolbar");
$pageContent->append($refreshToolbar);

$refreshBtn = DOM::create("div", "", "refreshModules", "toolbarTool");
...
while ($row = $dbc->fetch($modules)) 
{
	...
	if (count($queries) > 0)
	{
		...
		foreach ($queries as $queryID => $queryName)
		{
			...
			$attr['mid'] = $module_ID;
			$attr['qid'] = $queryID;
			$actionFactory->setModuleAction($fileItem, $innerModules['queryEditor'], "", "", $attr);
		}
	}
}
