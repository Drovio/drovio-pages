...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	$initLiterals = moduleLiteral::get($mID, "", array(), FALSE, locale::getDefault());
	...
	foreach ($newLiterals as $lName => $lValue)
	{
		...
		if (empty($initLiterals[$lName]))
			moduleLiteral::add($mID, $lName, $lValue);
		else 
		{
			if ($initLiterals[$lName] != $lValue)
				moduleLiteral::update($mID, $lName, $lValue);
	...
	foreach ($delete as $lName => $mixed)
		moduleLiteral::remove($mID, $lName);
}

$form = new simpleForm();

// Project ID
$input = $form->getInput($type = "hidden", $name = "mid", $value = $mID, $class = "", $autofocus = FALSE);
$pageContent->append($input);


// Get project literals
$projectLiterals = moduleLiteral::get($mID, "", array(), FALSE, locale::getDefault());
