...
$input = $form->getInput($type = "hidden", $name = "id", $value = $websiteID, $class = "", $autofocus = TRUE, $required = TRUE);
$form->append($input);
...
$input = $form->getInput($type = "text", $name = "domain[web_root]", $web_root, $class = "", $autofocus = FALSE, $required = FALSE);
...
// Website active template
$templateThemeResource = array();
...
foreach ($websiteTemplates as $templateName)
{
	...
	foreach ($themes as $themeName)
		$templateThemeResource[$templateName.".".$themeName] = $templateName." > ".$themeName;
}
$templateName_active = $wsSettings->get('template_name');
$templateThemeName_active = $wsSettings->get('template_theme_name');
$selectedValue = $templateName_active.".".$templateThemeName_active;
$title = moduleLiteral::get($moduleID, "lbl_activeTemplate");
$input = $form->getResourceSelect($name = "active_template", $multiple = FALSE, $class = "", $templateThemeResource, $selectedValue);
$form->insertRow($title, $input, $required = FALSE, $notes);
