...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	// Export Core SDK
	if (isset($_POST['coreSDK']))
	{
		$logDescription .= "Core SDK | ";
	...
	// Export AppCenter SDK
	if (isset($_POST['appcenterSDK']))
	{
		$logDescription .= "appCenter SDK | ";
		appLibrary::exportLibrary("ACL");
	}
	
	// Export eBuilder SDK
	if (isset($_POST['ebSDK']))
	{
		$logDescription .= "eBuilder SDK | ";
		//sdkManager::exportLibrary("API");
		//sdkManager::exportLibrary("UI");
		//sdkManager::exportLibrary("ESS");
		//sdkManager::exportLibrary("INU");
...
$title = moduleLiteral::get($moduleID, "lbl_sqlQueries");
$input = $form->getInput($type = "checkbox", $name = "sql", $value = "", $class = "", $autofocus = FALSE);
DOM::attr($input, "checked", "checked");
$form->insertRow($title, $input, $required = TRUE, $notes = "");

$title = moduleLiteral::get($moduleID, "lbl_coreSDK");
$input = $form->getInput($type = "checkbox", $name = "coreSDK", $value = "", $class = "", $autofocus = FALSE);
DOM::attr($input, "checked", "checked");
$form->insertRow($title, $input, $required = TRUE, $notes = "");

$title = moduleLiteral::get($moduleID, "lbl_appcenterSDK");
$input = $form->getInput($type = "checkbox", $name = "appcenterSDK", $value = "", $class = "", $autofocus = FALSE);
$form->insertRow($title, $input, $required = TRUE, $notes = "");

$title = moduleLiteral::get($moduleID, "lbl_ebSDK");
$input = $form->getInput($type = "checkbox", $name = "sbSDK", $value = "", $class = "", $autofocus = FALSE);
$form->insertRow($title, $input, $required = TRUE, $notes = "");
