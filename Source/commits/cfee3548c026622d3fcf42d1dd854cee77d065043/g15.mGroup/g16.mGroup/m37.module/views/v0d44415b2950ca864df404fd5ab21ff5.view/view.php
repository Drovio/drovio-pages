...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	$parent_id = $_POST['groupFamily'];
	$rootParent = $_POST['rootParent'];
	if (isset($rootParent))
		$parent_id = NULL;	
	
	//$module_group = new Ugroup();
	$module_group = new moduleGroup();
	$success = $module_group->create($description, $parent_id);
...
$frame->build("Create new Module Group", $moduleID, "createGroup", FALSE);

$sForm = new simpleForm();

// Header
$hdr = moduleLiteral::get($moduleID, "info.newCommandGroupPrompt");
$frame->append($hdr);

// Module Group Description
$title = moduleLiteral::get($moduleID, "lbl_groupDescription");
...
$moduleGroups = $dbc->toArray($moduleGroupsRaw, "id", "description");//to_array($moduleGroupsRaw, "id", "description");
$moduleGroups_depths = $dbc->toArray($moduleGroupsRaw, "id", "depth");//to_array($moduleGroupsRaw, "id", "depth");
...
$title = moduleLiteral::get($moduleID, "lbl_groupFamily");
$moduleGroupInput = $sForm->getResourceSelect($name = "groupFamily", $multiple = FALSE, $class = "", $moduleGroups, $selectedValue = NULL);
