...
foreach ($pData as $entry)
{
	...
	// Visits per Module
	if(array_key_exists($entry['moduleID'], $pgVisits))
	{
		$pgVisits[$entry['moduleID']]++;
	}
	else
	{
		$pgVisits[$entry['moduleID']] = 1;
	}
}
//-------------------------------------------------------------------------
...
// structured
	...
	$pgVisits = array_flip($pgVisits);
	krsort($pgVisits);
	foreach($pgVisits as $key=>$value)
	{
		$row = array();
		$row[] = "".$value;
		$row[] = "".$key;
