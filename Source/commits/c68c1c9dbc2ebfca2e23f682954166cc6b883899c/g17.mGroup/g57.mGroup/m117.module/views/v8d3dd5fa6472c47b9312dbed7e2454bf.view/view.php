...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	{	 		
		...
		$HTMLContent->addReportAction('delete.error', 'tSnip_'.$_POST['objectName']);
	...
	$HTMLContent = new HTMLContent();
	$HTMLContent->addReportAction('delete.success', 'tSnip_'.$_POST['objectName']);
	return $HTMLContent->getReport();
...
$ff = new formFactory();
$sForm->build($moduleID, "deleteTheme", $controls = FALSE);
// Append Form
...
$input = $sForm->getInput("hidden", "objectName", $themeName, $class = "", $autofocus = FALSE);
$sForm->append($input);

$content = DOM::create('div', '', '', 'content');
$sForm->append($content);
$confiramtionMsg = DOM::create('span', 'Are you sure deleting this item : '.$themeName); //moduleLiteral::get($moduleID, "lbl_literalManager");
DOM::append($content, $confiramtionMsg);
...
// Return output
$HTMLContent->addReportContent('delete.toggle', 'tSnip_'.$_GET['objectName']);
$HTMLContent->addReportAction('delete.toggle', 'tSnip_'.$_GET['objectName']);
