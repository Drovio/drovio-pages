...
foreach ($projectAccounts as $account)
{
	...
	$authorViewer = getContributorView($account['title'], $authDesc);
...
foreach ($authors as $authorID => $authorName)
{
	...
	$authorViewer = getContributorView($authorName, $authDesc);
	$pageContent->append($authorViewer);
}
...
function getContributorView($authorName, $authDesc)
{
	// Create commit viewer
	$authorViewer = DOM::create("div", "", "", "cViewer");
