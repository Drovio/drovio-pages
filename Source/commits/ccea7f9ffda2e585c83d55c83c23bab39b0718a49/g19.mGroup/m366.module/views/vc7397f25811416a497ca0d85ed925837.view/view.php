...
if (engine::isPost())
{
	...
	$teamUName = str_replace(" ", "_", $teamName);
	if (empty($teamUName))
	{
		$has_error = TRUE;
	...
	if (account::validate())
	{
		...
		// Set attributes and execute
		$attr = array();
		$attr['uname'] = strtolower($teamUName);
		$attr['name'] = $teamName;
	...
	else
	{
		...
		return $lg->build($regtype = registerDialog::REG_TYPE_PAGE, $return_url = "")->getReport($background = TRUE, $fade = TRUE);
	}
}
