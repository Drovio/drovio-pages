...
foreach ($locales as $locale)
{
	...
	$percentage = round(($localeCount / $totalCount) * 100, 2);
	...
	$content = DOM::create("div", "", "", "lcnt");
	DOM::append($lct, $content);
	
	$desc = DOM::create("span", $locale['uniDescription'], "", "lcdesc");
	DOM::append($content, $desc);
	
	$perc = DOM::create("span", $percentage." %", "", "lcperc");
	DOM::append($content, $perc);
	
	$sbar = DOM::create("div", "", "", "sb");
	DOM::append($lct, $sbar);
	
	// sbar content
	$sperc = DOM::create("span", "", "", "sp");
	DOM::append($sbar, $sperc);
	
	$perc = $percentage*100;
	DOM::attr($sperc, "style", "width: ".($perc/100)."%");
...
$totalPercentage = round(($totalLocaleCount / ($totalCount*$localeCountNum)) * 100, 2);
