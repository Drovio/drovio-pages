...
$description = HTML::select('.description > .content')->item(0);
...
$solution = HTML::select('.solution > .content')->item(0);
	...
	{
		$text = DOM::create();
		$rtext = DOM::create('div', '', '', 'noSolution msg_noContent'); 
		$literal = moduleLiteral::get($moduleID, "ntf_noSolutionYet");
		DOM::append($rtext, $literal);
		DOM::append($text, $rtext);
		
		$solveBug = module::loadview($moduleID, "solveBug");
		DOM::append($text, $solveBug);
		HTML::addClass($solveBug, 'noDisplay');
		HTML::addClass($solveBug, 'solveBugForm');
...
$comments = HTML::select('.comments > .content')->item(0);
	...
	{
		$text = DOM::create();
		$rtext = DOM::create('div', '', '', 'noComments msg_noContent'); 
