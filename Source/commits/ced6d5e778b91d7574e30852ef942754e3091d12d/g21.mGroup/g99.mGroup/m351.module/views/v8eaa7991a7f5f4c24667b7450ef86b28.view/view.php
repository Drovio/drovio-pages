...
// If not valid redirect back to relations
if (!$validTeam)
{
	$url = url::resolve("my", "/relations/");
...
if (empty($teamName) && !empty($teamInfo['uname']))
{
	...
	$url = url::resolve("my", "/relations/".$teamInfo['uname']);
	return $actionFactory->getReportRedirect($url, "", $formSubmit = TRUE);
}
...
$leaveButton = HTML::select(".teamInfoContainer .logoBox .leaveTeam")->item(0);
if (!$accountInfo['administrator'])
{
	// Remove 'leave team' button
	HTML::replace($leaveButton, NULL);
	
	// Remove members tab
	unset($items['members']);
	$navItem = HTML::select(".teamNavigation .navitem.members")->item(0);
	HTML::replace($navItem);
}
else
{
	...
	$attr = array();
	$attr['tid'] = $teamID;
	$actionFactory->setModuleAction($leaveButton, $moduleID, "leaveTeam", "", $attr);
}
