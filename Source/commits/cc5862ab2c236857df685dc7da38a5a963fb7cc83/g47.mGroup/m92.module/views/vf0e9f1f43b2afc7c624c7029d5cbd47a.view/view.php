...
$page->build($title, "appListContainer", TRUE);
$appContainer = HTML::select(".appListContainer .appList")->item(0);
...
function getAppBox($app, $page, $moduleID, $innerModules, $actionFactory)
{
	...
	// Create box upper part
	$appBoxMain = DOM::create("div", "", "", "bmain");
	...
	$appTitle = $page->getWebLink($url, $app['title'], "_self", NULL, "", array(), "appTitle");
	...
	$attr['id'] = $app['id'];
	$attr['name'] = $app['name'];
	$actionFactory->setModuleAction($appTitle, $innerModules['appInfo'], "applicationInfoViewer", ".appCenterContentHolder", $attr);
	
	// Owner Team
	$appDevs = DOM::create('div', '', '', 'appOwner');
	$inner = DOM::create("span", $app["teamName"], "", "");
