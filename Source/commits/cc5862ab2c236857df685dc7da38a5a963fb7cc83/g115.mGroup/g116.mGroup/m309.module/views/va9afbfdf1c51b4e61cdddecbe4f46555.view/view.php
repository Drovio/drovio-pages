...
if (empty($projectName) && !empty($projectInfo['name']))
{
	...
	$url = url::resolve("developers", "/open/projects/".$projectName);
...
if (!$projectInfo['open'])
{
	...
	$url = url::resolve("developers", "/open/projects/");
	return $actionFactory->getReportRedirect($url, $domain = "", $formSubmit = FALSE);
}
...
if (!empty($projectName))
	$projectUrl = url::resolve("developers", "/open/projects/".$projectName);
else
{
	$params = array();
	$params['id'] = $projectID;
	$projectUrl = url::resolve("developers", "/open/projects/project.php", $params);
}
...
// Load footer menu
$openProjectContainer = HTML::select(".openProject")->item(0);
