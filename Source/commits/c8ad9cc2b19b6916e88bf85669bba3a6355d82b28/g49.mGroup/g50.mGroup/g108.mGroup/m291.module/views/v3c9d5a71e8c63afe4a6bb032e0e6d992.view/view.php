...
{
	// Skip current object
	if ($sObject['namespace'] == $namespace && $sObject['name'] == $objectName)
		continue;
	...
	// Title
	$href = url::resolve("developer", "/docs/sdk/".$objectDomain."/".$sObject['library']."/".$sObject['package']."/".(empty($sObject['namespace']) ? "" : $sObject['namespace']."/").$sObject['name']);
	...
	// Full namespace
	$sObject['namespace'] = str_replace("::", "\\", $sObject['namespace']);
	$fullName = $sObject['library']."\\".$sObject['package']."\\".(empty($sObject['namespace']) ? "" : $sObject['namespace']."\\").$sObject['name'];
