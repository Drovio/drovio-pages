...
$innerModules['devPlanManager'] = 382;
...
$teamAdmin = apiKey::validateGroupName($groupName = "TEAM_ADMIN", $accountID, $teamID, $projectID = NULL);

// Get all account project keys
$accountKeys = apiKey::getAccountKeys($accountID = NULL);
$accountProjectKeys = array();
foreach ($accountKeys as $keyInfo)
	if (!empty($keyInfo['project_id']))
		$accountProjectKeys[$keyInfo['project_id']][] = $keyInfo;

// Check if there is an active team
$newProjectButton = HTML::select(".projectContainer .navBar .wbutton.create")->item(0);
$planIco = HTML::select(".projectContainer .navBar .plan")->item(0);
if (empty($teamID))
{
	// Get team name
	...
	// Remove new project button and plan ico
	HTML::replace($newProjectButton, NULL);
	HTML::replace($planIco, NULL);
}
else
{
	// Get team name
	...
	// Add new project button action
	$actionFactory->setModuleAction($newProjectButton, $moduleID, "projectWizard");
	
	// Set plan ico
	$teamPlanID = devTeam::getCurrentPlan($live = TRUE);
	$plan_class = devTeam::getPlanName($teamPlanID);
	$plan_title = moduleLiteral::get($moduleID, "lbl_plan_".$plan_class);
	HTML::addClass($planIco, $plan_class);
	HTML::append($planIco, $plan_title);
	if ($teamAdmin)
	{
		HTML::addClass($planIco, "editable");
		$actionFactory->setModuleAction($planIco, $innerModules['devPlanManager'], "");
	}
}
...
foreach ($allProjects as $project)
{
	...
	switch ($project['projectType'])
	{
		...
		case 2:
		case 3:
			HTML::addClass($projectBox, "platform");
			$red_count++;
	...
	if (isset($myProjects[$project['id']]) || isset($sharedProjects[$project['id']]))
	{
		...
		{
			$url = url::resolve("developers", "/dashboard/project.php");
			$params = array();
		...
		else
		{
			...
			$url = url::resolve("developers", "/dashboard/".$project['name']);
		}
	...
	// Team Name
	$teamInfo = team::getTeamInstance()->info($project['team_id']);
	...
	// Project Account Permissions
	$permissions = "";
	foreach ($accountProjectKeys[$project['id']] as $perList)
		$permissions .= $perList['user_group_name'].", ";
	$permissions = trim($permissions, ", ");
	$pTypeElement = DOM::create("h3", $permissions, "", "pinfo prole");
	HTML::append($projectBoxContainer, $pTypeElement);
	...
	$open = moduleLiteral::get($moduleID, "lbl_overviewProject");
	$projectLink = $page->getWeblink($url, $open, "_self", $innerModules['projectHome'], "", $attr);
	$boxTitle = DOM::create("div", $projectLink, "", "ba overview");
	...
	if (isset($myProjects[$project['id']]) || isset($sharedProjects[$project['id']]))
	{
		...
		if (empty($project['name']))
		{
			...
			$url = url::resolve("developers", "/dashboard/".$project['name']."/designer/");
		...
		$slico = DOM::create("div", "", "", "slico");
		DOM::append($projectLink, $slico);
	}
}
