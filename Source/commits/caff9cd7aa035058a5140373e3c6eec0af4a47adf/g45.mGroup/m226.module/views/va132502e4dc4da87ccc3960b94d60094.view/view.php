...
if (count($apps) > 0)
{
	...
	$active = 0;
	$activeSlide = HTML::select(".slideContainer .bslide")->item($active);
	...
	foreach ($apps as $app)
	{
		...
		$appsGrid = HTML::select(".apps_grid .bslide .grid")->item($appSlide);
		
		// Get application ico
		$appIcon = projectLibrary::getPublishedPath($app['application_id'], $app['version'])."/resources/ico.png";
		$appIcon2 = projectLibrary::getPublishedPath($app['application_id'], $app['version'])."/resources/.assets/icon.png";
		...
		else if (file_exists(systemRoot.$appIcon2))
		{
			$appTileIcon = str_replace(paths::getPublishedPath(), "", $appIcon2);
			$appTileIcon = url::resolve("lib", $appTileIcon);
		}
		else
			$appTileIcon = "";
		...
		$applicationData['id'] = $app['application_id'];
...
if (empty($teamID))
{
	...
	// Get team container
	$teamContainer = HTML::select(".bossDashboard .teamChooser .teams")->item(0);
...
else
{
	...
	$teamChooser = HTML::select(".bossDashboard .teamChooser")->item(0);
...
function setNavBalls()
{
	...
	$numSlides = HTML::select(".slideContainer .bslide")->length;
