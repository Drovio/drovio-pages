...
if (engine::isPost())
{
	...
	$status = team::updateInfo($_POST['name'], $_POST['description'], $uname, $teamID);
...
// Build form
$form = new simpleForm();
$editorForm = $form->build($action = "", $defaultButtons = TRUE, $async = TRUE)->engageModule($moduleID, "editInfo")->get();
...
$input = $form->getInput($type = "text", $name = "uname", $value = $teamInfo['uname'], $class = "tinp fLeft", $autofocus = TRUE, $required = TRUE);
$fRow = $form->buildRow($title, $input, $required = TRUE, $notes);
$form->append($fRow);

$notes = moduleLiteral::get($moduleID, "lbl_info_uname_ph");
HTML::addClass($notes, "tnotes");
DOM::append($fRow, $notes);

// Team name
$title = moduleLiteral::get($moduleID, "lbl_info_name");
$input = $form->getInput($type = "text", $name = "name", $value = $teamInfo['name'], $class = "", $autofocus = FALSE, $required = TRUE);
