...
{
	// Set nav item
	$item = HTML::select(".docNavigation .hd.".$class)->item(0);
	NavigatorProtocol::staticNav($item, $ref, $targetcontainer, $targetgroup, $navgroup, $display = "none");
...
if (!empty($classInfo['deprecated']))
{
	// Create deprecated notification
	$ntf = new notification();
	$depNtf = $ntf->build("warning", $header = TRUE, $timeoud = FALSE, $disposable = FALSE);
	...
	$container = HTML::select(".docNavigation")->item(0);
	$depNtf = $ntf->get();
	DOM::appendBefore($container->parentNode, $container, $depNtf);
}
...
$manual = fileManager::get(systemRoot.$manualFilePath);
if (!empty($manual))
