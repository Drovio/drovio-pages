...
$pageContent->build("", "accountsToGroupsPage", TRUE);
$actionFactory = $pageContent->getActionFactory();

// Sidebar
$sidebar = HTML::select(".accountsToGroupsPage .accList")->item(0);
$sideMenu = new treeView();
$usersMenu = $sideMenu->build()->get();
...
while ($row = $dbc->fetch($allAccounts)) 
{
	...
	$attr = array();
	$attr['aid'] = $row['id'];
	$actionFactory->setModuleAction($itemTitle, $moduleID, "accGroupEditor", ".accEditor", $attr, $loading = TRUE);
}
