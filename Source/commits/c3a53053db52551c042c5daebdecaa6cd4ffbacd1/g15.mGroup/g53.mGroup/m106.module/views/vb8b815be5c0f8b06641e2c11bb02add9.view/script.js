jq(document).one("ready", function() {
	// Switches
	jq(document).on("status.modified", "#testSwitch", function() {
	...
	jq(document).on("status.modified", "#loggerSwitch", function() {
		if (switchButton.getStatus(jq(this)))
			jq("#loggerDataContainer").removeClass("noDisplay");
		else
			jq("#loggerDataContainer").addClass("noDisplay");
	});
	...
	jq(document).on("content.modified", function() {
		...
		for (key in resources) {
			...
			if (resources[key].type == "css")
				jqLi.appendTo(jq(".resourceListContainer .cssResources"));
			else
				jqLi.appendTo(jq(".resourceListContainer .jsResources"));
		}
	});
	
	// Resource Management listeners
	jq(document).on("click", ".rsrcReload", function() {
	...
	jq(document).on("click", ".rsrcRemove", function() {
		// Get closest boot resource
		var jqResource = jq(this).closest(".bootResource");
