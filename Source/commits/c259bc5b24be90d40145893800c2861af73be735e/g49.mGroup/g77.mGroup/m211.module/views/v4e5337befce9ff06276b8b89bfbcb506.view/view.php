...
if (engine::isPost())
{
	...
	if ($status)
	{
		...
		$errorMessage = DOM::create("h2", "Invitation revoked.");
		$succFormNtf->append($errorMessage);
		
		// Add action to delete row
		$succFormNtf->addReportAction($type = "project_invitations.remove", $refID);
	...
	else
	{
		...
		// Notification Message
		$errorMessage = DOM::create("h2", "An error occurred.");
		$succFormNtf->append($errorMessage);
