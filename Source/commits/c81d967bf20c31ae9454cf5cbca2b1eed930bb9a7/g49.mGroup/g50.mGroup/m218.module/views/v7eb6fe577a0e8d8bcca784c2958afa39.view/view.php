...
$manualFilePath = $docRoot.$objectDomain."/".$objectPath."/".$objectName.".manual.html";
...
$container = $pageContent->build("", "sdkManual", TRUE)->get();
...
$targetcontainer = "DocBody";
$targetgroup = "manGroup";
$navgroup = "manavGroup";

$item = HTML::select(".bodyDetailsContainer .hd.manual")->item(0);
NavigatorProtocol::staticNav($item, "docManual", $targetcontainer, $targetgroup, $navgroup, $display = "none");

$item = HTML::select(".bodyDetailsContainer .hd.synopsis")->item(0);
NavigatorProtocol::staticNav($item, "docSynopsis", $targetcontainer, $targetgroup, $navgroup, $display = "none");

// targetgroup
$item = HTML::select("#docManual")->item(0);
$pageContent->setNavigationGroup($item, $targetgroup);

$item = HTML::select("#docSynopsis")->item(0);
$pageContent->setNavigationGroup($item, $targetgroup);
...
$version = DOM::create("span", (empty($classInfo['version']) ? "0.0-0" : $classInfo['version']."-".$classInfo['build']));
...
$live = datetimer::live($classInfo['datecreated']);
$box = HTML::select(".boxInfo .created")->item(0);
DOM::append($box, $literal);
DOM::append($box, $live);

$literal = moduleLiteral::get($moduleID, "lbl_updatedAt");
$live = datetimer::live($classInfo['daterevised']);
$box = HTML::select(".boxInfo .updated")->item(0);
DOM::append($box, $literal);
DOM::append($box, $live);
...
$manual = fileManager::get($manualFilePath);
