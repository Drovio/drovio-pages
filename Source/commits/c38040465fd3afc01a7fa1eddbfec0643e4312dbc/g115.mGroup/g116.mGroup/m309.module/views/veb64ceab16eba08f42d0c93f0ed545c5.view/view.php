...
foreach ($projectAccounts as $account)
{
	...
	$authorViewer = getContributorView($account['accountID'], $account['title'], $authDesc);
...
foreach ($authors as $authorID => $authorName)
{
	...
	$authorViewer = getContributorView($authorID, $authorName, $authDesc);
	$pageContent->append($authorViewer);
}
...
function getContributorView($authorID, $authorName, $authDesc)
{
	// Create commit viewer
	$authorViewer = DOM::create("a", "", "", "cViewer");
	DOM::attr($authorViewer, "target", "_blank");
	$params = array();
	$params['id'] = $authorID;
	$url = url::resolve("developer", "/profile/index.php");
	$url = url::get($url, $params);
	DOM::attr($authorViewer, "href", $url);
