...
while ($row = $dbc->fetch($modules)) 
{
	...
	$moduleFileItem = $treeView->insertSemiExpandableTreeItem('m'.$module_ID, $file, "mg".$row['group_id']);
	
	// Set action to module info
	$attr = array();
	$attr['mid'] = $module_ID;
	$actionFactory->setModuleAction($moduleFileItem, $innerModules['viewEditor'], "moduleInfo", "", $attr);
