...
$attr = array();
$attr['id'] = $projectID;
$actionFactory->setModuleAction($pStatus, $moduleID, "publishProject", "", $attr);
...
foreach ($actions as $action)
	setSectionAction($moduleID, $actionFactory, $projectID, $action, $innerModules[$action], "", $selectedTab);
...
function setSectionAction($moduleID, $actionFactory, $projectID, $tab, $actionID, $actionName = "", $selectedTab = "overview")
{
	if (empty($actionID))
		return;
	...
	if ($tab == $selectedTab)
	{
		...
		// Load content
		$content = module::loadView($actionID, $actionName);
