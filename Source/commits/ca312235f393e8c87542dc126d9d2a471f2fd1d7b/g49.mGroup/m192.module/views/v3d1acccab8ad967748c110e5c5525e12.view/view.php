...
$statusContent = HTML::select(".content")->item(0);
...
if ($rbDeveloper)
{
	$deployContainer = HTML::select(".content")->item(1);
	...
	$attr = array();
	$attr['date'] = date('d F Y \o\n H:i:s', $latest_log['time']);
	$dateTitle = moduleLiteral::get($moduleID, "lbl_lastDeployDate", $attr);
...
// Publish status
$publishContainer = HTML::select(".content")->item(2);
foreach ($projects as $projectName => $projectVersions)
...
$attr = array();
$attr['date'] = date('d F Y \o\n H:i:s', $latest_log['time']);
$dateTitle = moduleLiteral::get($moduleID, "lbl_lastPublishDate", $attr);
$tDate = DOM::create("div", $dateTitle, "", "tDate");
DOM::append($publishContainer, $tDate);


// Issue History
$errorLogsContainer = HTML::select(".content")->item(3);
