...
// Get Account Keys
$akeys = apiKey::getAccountKeys();

// Get team and project keys
$teamKeys = array();
$projectKeys = array();
foreach ($akeys as $akey)
	if (isset($akey['team_id']))
		$teamKeys[] = $akey;
	else
		$projectKeys[] = $akey;
...
function getKeyList($keyList, $moduleID, $headers)
{
	...
	foreach ($keyList as $key)
	{
		...
		// Get key context description
		if (isset($key['team_id']))
			$description = accountKey::getContextDescription(accountKey::TEAM_KEY_TYPE, $key['team_id']);
		else
			$description = accountKey::getContextDescription(accountKey::PROJECT_KEY_TYPE, $key['project_id']);
		$context = (isset($key['team_id']) ? $key['team_id'] : $key['project_id']);;
		$row[] = $context;
		$row[] = $description;
		$row[] = $key['user_group_name'];
		$class = "k".$key['user_group_id'].$key['type_id'].$context;
