...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	$success = $bugger->assign($_POST['bid'], $_POST['accountId']);
...
// Build the module 
$MContent->build("", "assignBug inlineForm", TRUE);

$formContent = HTML::select('.assignBug .actionForm ')->item(0);

$sForm = new simpleForm();
$sForm->build($moduleID, "assignBug", FALSE);
...
{
	$resource[$member['accountID']] = $member['firstname']." ".$member['lastname'];
}
$input = $sForm->getResourceSelect("accountId", $multiple = FALSE, $class = "", $resource, '');
$icBlock = DOM::create('div', '', '', 'icBlock');
DOM::append($icBlock, $input);
$sForm->append($icBlock);

$icBlock = DOM::create('div', '', '', 'icBlock');
$button = $sForm->getSubmitButton(moduleLiteral::get($moduleID, "btn_assign"));
