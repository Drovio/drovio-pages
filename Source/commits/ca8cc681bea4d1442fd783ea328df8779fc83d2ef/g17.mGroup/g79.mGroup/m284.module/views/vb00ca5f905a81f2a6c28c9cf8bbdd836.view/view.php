...
if (engine::isPost())
{
	...
	logger::log("Connecting to server ".$serverInfo['username']."@".$serverInfo['address'].": ".($status ? "SUCCESS" : "FAILURE"), logger::DEBUG);
	if (!$status)
		return uploadError($pageContent, $errorContent = "Could not connect to the server.", $errorLog = "Could not connect to the server.", $step);
	...
	if (!$status)
		return uploadError($pageContent, $errorContent = "Could not upload the preparation files.", $errorLog = "Could not upload the preparation files.", $step);
	...
	if (empty($response) || !$responseArr['status'])
		return uploadError($pageContent, $errorContent = "Server preparation process failed. Please try again.", $errorLog = "Server preparation process failed.", $step);
	...
	if (!$status)
		return uploadError($pageContent, $errorContent = "Could not upload the setup files. Please try again.", $errorLog = "Could not upload the setup files.", $step);
	
	// Upload web core to .temp folder
	$coreVersion = $wsSettings->get('core_version');
	$coreVersion = (empty($coreVersion) || !$coreVersion || $coreVersion < 0 ? NULL : $coreVersion);
	$webCorePath = webCoreProject::getDistroPath($coreVersion = "", $rootRelative = FALSE);
	$status = $ftp->put($webCorePath, ".temp/wsdk.zip", "");
	logger::log("Uploading core zip file : ".($status ? "SUCCESS" : "FAIL"), logger::DEBUG);
	if (!$status)
		return uploadError($pageContent, $errorContent = "An error occurred while uploading the website core. Please try again.", $errorLog = "An error occurred while uploading the website core.", $step);
	...
	if (empty($response) || !$responseArr['status'])
		return uploadError($pageContent, $errorContent = "Setup process failed. Please try again.", $errorLog = "Setup process failed.", $step);
	
	// Pack the latest website release
	$publishFolder = systemRoot.projectLibrary::getPublishedPath($websiteID, $releaseVersion);
	logger::log("Website publish folder: ".$publishFolder, logger::DEBUG);
	...
	if (!$status)
		return uploadError($pageContent, $errorContent = "An error occurred while uploading the website files. Please try again.", $errorLog = "An error occurred while uploading the website files.", $step);
...
function uploadError($pageContent, $errorContent, $errorLog, $step)
{
	// Log error
	logger::log($errorLog, logger::DEBUG);

	// Add error action
	$pageContent->addReportAction("website.error", $step);

	// Add error content
	if (is_string($errorContent))
		$errorContent = DOM::create("span", $errorContent);
	$pageContent->append($errorContent);
...
{
	// Log
	logger::log("doRequest : ".$url, logger::DEBUG);
	...
	// Clean non-json characters
	$lastBracket = strrpos($response, $needle = "}");
	$response = substr($response, 0, $lastBracket + 1);
	
	// Log
	logger::log("doRequest response : ".$response, logger::DEBUG);
	logger::log("doRequest error : ".$error, logger::DEBUG);
