...
dashboard = {
	...
	init: function() {
		...
		jq(document).on("click", ".slideContainer .bslide .app", function() {
		...
		// Resize app
		jq(document).on("click", ".slideContainer .bslide .app .size", function(ev) {
			// Stop bubbling
			ev.stopPropagation();
		...
		jq(document).on("click", ".dashSettings .settings.ico", function() {
			// Get settings dialog
			var jqSettingsDialog = jq(".settings_popup").clone(true).removeClass("noDisplay");
			
			// Show popup
			jq(this).popup.position = "bottom|center";
			jq(this).popup(jqSettingsDialog);
			return;
		});
		
		// Change theme
		jq(document).on("click", ".thm_tile", function() {
			// Get theme id
			var themeID = jq(this).data("thm");
			if (themeID == "")
				return;
				
			// Get theme template
			var themeTemplate = jq(".apps_grid .thm").first().clone().removeAttr("class").addClass("thm");
			
			// Add theme to themes
			jq(".apps_grid .thm").last().after(themeTemplate);
			
			// Add wait time for transition and remove old theme
			setTimeout(function() {
				themeTemplate.addClass(themeID);
				setTimeout(function() {
		...
		// Change color
		jq(document).on("click", ".clr_tile", function() {
			// Get theme id
			var colorHex = jq(this).data("clr");
			if (colorHex == "")
				return;
				
			// Set all colors at the same time
			jq(".thm .clr").removeAttr("style").attr("style", "background: "+colorHex);
		});
	},
	...
	setSlides: function() {
		...
		jq(".bslide").css("padding", vPadding + "px " + hPadding + "px");
	},
	scrollTo: function(index) {
		...
		var slideHeight = jq(".bslide").first().outerHeight();
		var marginTop = -slideHeight * index;
		jq(".bslide").first().animate({
