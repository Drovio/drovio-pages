...
if (engine::isPost())
{
	...
	switch ($projectInfo['projectType'])
	{
		...
		case 2: // Publish Redback Modules
			moduleManager::publish($branchName);
			
			// Generate sitemap
			sitemap::generate();
			
			// Publish Resources
			$project->publishResources("/Library/Media/m/", $innerFolder = "/media/");
			$project->publishResources("/System/Resources/Modules/", $innerFolder = "/resources/");
		...
		case 3: // Publish Redback Web Engine SDK			
			webManager::publish($releaseVersion, $branchName);
		...
		case 4:
			...
			$app->publish($releaseVersion, $branchName);
		...
		case 5:	// Publish Website
			...
			$website->publish($releaseVersion, $branchName);
...
$input = $form->getResourceSelect($name = "branch", $multiple = FALSE, $class = "", $branchResource, $selectedValue = "");
$form->insertRow($title, $input, $required = TRUE, $notes = "");
