...
{
	// Build module page
	$page->build("", "websiteDesingerPage", TRUE);
	
	// Add error message
	$ntf = new notification();
	$ntf->build($type = notification::ERROR, $header = TRUE, $timeout = FALSE, $disposable = FALSE);
	$message = moduleLiteral::get($moduleID, "ntf_projectType_error");
	$ntf->append($message);
	
	$webContainer = HTML::select(".websiteDesingerPage .webContainer")->item(0);
	DOM::append($webContainer, $ntf->get());
