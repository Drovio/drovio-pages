...
$packageExplorer = HTML::select(".sdkManualContainer #webSDK")->item(0);
...
function buildObjTree($navTree, $libName, $packageName, $parentNs, $sourceMap, $domain)
{
	...
	foreach ($objs as $key => $value)
	{
		...
		// Set href attributes
		$url = url::resolve("developer", "/docs/sdk/".$domain."/".$libName."/".$packageName."/".(empty($parentNs) ? "" : $parentNs."/").$value['name']);
