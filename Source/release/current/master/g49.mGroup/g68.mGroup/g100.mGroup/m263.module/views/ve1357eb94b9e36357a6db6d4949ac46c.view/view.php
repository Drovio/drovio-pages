...
// New package
$title = moduleLiteral::get($moduleID, "lbl_createPackageTitle");
...
// Library Item
$item = DOM::create("div", "", "", "srcLibrary");
$itemIco = DOM::create("span", "", "", "contentIcon libIco");
DOM::append($item, $itemIco);
$itemName = DOM::create("span", srcPackage::LIB_NAME);
DOM::append($item, $itemName);

// Add tree item
$item = $navTree->insertExpandableTreeItem(srcPackage::LIB_NAME, $item);
...
foreach ($packages as $packageName => $value)
{
	...
	// Add tree item
	$item = $navTree->insertExpandableTreeItem($packageName, $item, srcPackage::LIB_NAME);
