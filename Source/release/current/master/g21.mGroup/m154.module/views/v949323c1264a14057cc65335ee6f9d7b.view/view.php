...
$currentAccountInfo = account::getInstance()->info();
...
$accounts = array();
$accounts[] = account::getInstance()->info();
...
if (empty($currentAccountInfo['username']))
{
	// Build form
	$form = new simpleForm();
	...
	DOM::data($cancelButton, "skip", "profile_info_skip_username");
...
if (empty($currentAccountInfo['profile_image_url']))
{
	// Build form
	$form = new simpleForm();
	...
	$cancelButton = $form->getResetButton($title, $id = "", $class = "bbtn skip");
	$form->appendControl($cancelButton);
	DOM::data($cancelButton, "skip", "profile_info_skip_profileimage");
