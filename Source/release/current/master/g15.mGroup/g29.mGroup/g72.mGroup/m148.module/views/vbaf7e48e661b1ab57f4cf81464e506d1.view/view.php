...
// Title
$title = DOM::create("h2", "Region Listing");
...
$regionGridList = $gridList->build($id = "regionGridList", $checkable = FALSE)->get();
...
while ($row = $dbc->fetch($result))
{
	...
	$attr = array();
	$attr['rid'] = $row['id'];
	$actionFactory->setModuleAction($btnEdit, $moduleID, "editRegion", "", $attr);
