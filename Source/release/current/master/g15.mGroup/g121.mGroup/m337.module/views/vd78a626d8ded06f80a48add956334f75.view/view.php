...
$ce = new codeMirror($type = codeMirror::PHP, $name = "pageContent");
$pageCoder = $ce->build($pageContent)->get();
...
// Float toolbar
$title = moduleLiteral::get($moduleID, "lbl_pageAttr_float_toolbar");
$notes = moduleLiteral::get($moduleID, "lbl_pageAttr_float_toolbar_notes", array(), FALSE);
$input = $form->getInput($type = "checkbox", $name = "module[attr][float_toolbar]", $value = "1", $class = "", $autofocus = FALSE);
if ($pageData['attributes']['float_toolbar'])
	DOM::attr($input, "checked", TRUE);
$fRow = $form->buildRow($title, $input, $required = FALSE, $notes);
DOM::append($moduleWrapper, $fRow);
