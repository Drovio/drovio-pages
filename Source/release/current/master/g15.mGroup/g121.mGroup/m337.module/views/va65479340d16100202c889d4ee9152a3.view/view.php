...
if (engine::isPost())
{
	...
	if (!$success)
	{
		$err_header = moduleLiteral::get($moduleID, "lbl_folder");
		$err = $errFormNtf->addHeader($err_header);
		$errFormNtf->addDescription($err, DOM::create("span", "Error creating folder..."));
		return $errFormNtf->getReport();
	}
...
$input = $form->getResourceSelect($name = "parent", $multiple = FALSE, $class = "", $folderResource, $selectedValue = $currentFolderID);
