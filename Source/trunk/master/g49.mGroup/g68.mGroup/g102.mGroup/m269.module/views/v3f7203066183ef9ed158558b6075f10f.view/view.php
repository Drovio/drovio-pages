...
HTML::addClass($objectTabberControl, "appViewTabControl");
...
// PHP Editor
$editor = new codeMirror($type = codeMirror::PHP, "viewSource");
$code = trim($appView->getPHPCode());
$phpEditor = $editor->build($code, "", "cmEditor")->get();
HTML::addClass($phpEditor, "codeEditorContainer");
...
// Javascript Editor
$editor = new codeMirror($type = codeMirror::JS, "viewScript");
$code = trim($appView->getJS());
$jsEditor = $editor->build($code, "", "cmEditor")->get();
...
$glist = $dtGridList->build("", TRUE, FALSE)->get();
...
foreach ($appManifests as $mfID)
{
	...
	{
		$openList = (isset($openPackageList[$libName]) ? $openPackageList[$libName] : array());
		$openPackageList[$libName] = array_merge($openList, $mfInfo['packages'][$libName]);
	}
}
