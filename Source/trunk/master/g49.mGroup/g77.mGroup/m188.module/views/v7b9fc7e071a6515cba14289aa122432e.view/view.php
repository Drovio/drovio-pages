...
$releasePackagesContainer = HTML::select(".vcsReleases .releasePackagesContainer")->item(0);
...
foreach ($packages as $packageID => $packageData)
{
	...
	// Add history view button
	$title = moduleLiteral::get($moduleID, "lbl_showHistory");
	$showHistory = DOM::create("div", $title, "", "showHistory");
	DOM::append($packageRow, $showHistory);
	
	// Set navigation
	$pageContent->setStaticNav($showHistory, "", "", "", "packageGroup", $display = "none");
	...
	$attr['pid'] = $packageID;
	$actionFactory->setModuleAction($showHistory, $moduleID, "releaseBuilds", ".vcsReleases .releaseBuildsContainer", $attr, $loading = TRUE);
	...
	$version = DOM::create("span", ", ".$latestRelease['version']."-".$latestRelease['build'], "", "releaseVersion");
	DOM::append($releaseTitle, $version);
