...
if (engine::isPost())
{
	...
	// Check if the owner team is the same
	if ($_POST['tid'] != $projectInfo['team_id'] && team::getTeamID() == $projectInfo['team_id'])
		$project->updateTeam($_POST['tid']);
...
// Create Form
$form = new simpleForm();
$settingsForm = $form->build($action = "", $defaultButtons = TRUE, $async = TRUE, $fileUpload = TRUE)->engageModule($moduleID, "projectInfo")->get();
...
$input = $form->getFileInput($name = "project_icon", $class = "", $required = FALSE, $accept = ".png");
...
// Switch project to new team
if (team::getTeamID() == $projectInfo['team_id'])
{
	$teams = team::getAccountTeams();
	$teamResource = array();
	foreach ($teams as $teamInfo)
		$teamResource[$teamInfo['id']] = $teamInfo['name'];
	$title = moduleLiteral::get($moduleID,"lbl_projectTeam");
	$notes = moduleLiteral::get($moduleID, "lbl_projectTeam_notes", array(), FALSE);
	$input = $form->getResourceSelect($name = "tid", $multiple = FALSE, $class = "", $teamResource, $selectedValue = $projectInfo['team_id']);
	$form->insertRow($title, $input, FALSE, $notes);
}
