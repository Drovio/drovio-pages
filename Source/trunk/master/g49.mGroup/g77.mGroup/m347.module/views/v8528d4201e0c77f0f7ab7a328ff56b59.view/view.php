...
$pageContent->build("", "projectLiteralTranslationsContainer", TRUE);

// Check if there is a translator profile
$translatorProfile = translator::profile();
if (empty($translatorProfile))
{
	$literalNav = HTML::select(".projectLiteralTranslations .literalNav")->item(0);
	HTML::replace($literalNav, NULL);
	
	$transContent = HTML::select(".projectLiteralTranslations .transContent")->item(0);
	HTML::replace($transContent, NULL);
	
	return $pageContent->getReport();
}
else
{
	$hd = HTML::select(".projectLiteralTranslations .hd.no_translator")->item(0);
	HTML::replace($hd, NULL);
}
